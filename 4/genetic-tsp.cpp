...
bool findCity(const vector<int>& tour, int city) {
    ...
    // during evolution, tours temporarily have undefined slots, so this is why we need that function
    for(int i=0; i<N; ++i) {
...
//calculates the length for a given tour, assuming a round trip
int tourLength(const vector<int>& T) {
    int result = 0;
    int firstCity = *T.begin();
	for (auto city = T.begin(); city != T.end(); ++city)
    {
        result += distance_table[firstCity][*city];
        firstCity = *city;
    }
    result += distance_table[firstCity][*T.begin()];
    return result;
...
// Generates the tours as initial population and stores them in the tour set.
void generateTours(vector< vector<int> >& tourSet) {
    for(int j=0; j<M; j++)
    {
        ...
        std::shuffle(cities.begin(),cities.end(),std::default_random_engine(frand(0,500)));
        tourSet[j]=cities; 
    }
}

// takes two (good) parent tours, and builds a new one by the gens of both.
void crossover(const vector<int>& parent1, const vector<int>& parent2, vector<int>& child) {
    child = NullTour;
    int counter = int(frand(0,N-1));
   
    while(!tourDefined(child)){     
        for (int i = counter; i<counter+int(frand(0,10)); i++){
            if (findCity(child, parent1[i%N]))
                break;
            insertCity(child, parent1[i%N]);
        }
        for (int i = counter; i<counter+int(frand(0,10)); i++){
            if (findCity(child, parent2[i%N])){
                break;
            }
            insertCity(child, parent2[i%N]);
        }
        counter = (counter+int(frand(0,N-1)))%N;
    }
}

//Mutates a given tour, swapping cities randomly based on probability.
void mutate(vector<int>& tour) {
    const float mutationProbability = 0.02f; // x% probability per city in a tour to get mutated
    for (auto &t : tour){
        if (frand(0,100)<mutationProbability*100)
        {
            std::swap(t, tour[frand(0,19)]);
        }
    }
}

// Creates a sorted sequence of (tour length, tour index) pairs
vector<pair<int,int>> fitness(vector<vector<int>>& tourSet) {
    vector<pair<int,int>> F(M);
    for (int i=0; i<M; i++){
        F[i] = {tourLength(tourSet[i]),i};
    }
    std::sort(F.begin(), F.end(), [](const std::pair<int,int> &left, const std::pair<int,int> &right) {
    return left.first < right.first;
    });
    return F;
}

// evolution step: transforms the tour set into the next generation tour set
pair<int,int> evolution(vector<vector<int>>& tourSet, bool elite) {
	...
	// computes fitness of tours, store shortest and largest tour length in statistics
    ...
    if (statistics.first<minDist)
    {
        minDist=statistics.first;
    }
    if (statistics.second>maxDist)
    {
        maxDist=statistics.second;
    }

	// computes crossover of two best tours and replace worst tour by the crossover.
    crossover(tourSet[F[0].second], tourSet[F[1].second], tourSet[F[M-1].second]);
	// Mutates all other tours
    for (int i = 2; i < M-1; i++)
    {
