...
{
	/*bool moreThanOneStop = [](std::pair<int,int> destNumStops) {
		if(destNumStops.second>0)
			return true;
		else
			return false;
	};*/
	for(auto it = airportInfo.begin(); it!=airportInfo.end(); it++){
		...
		std::remove_if(it->second.m_routes.begin(), it->second.m_routes.end(), [](std::pair<int,int> paar){ return paar.second!=0; });
	}
...
void calculateDistancePerRoute(std::map<int, AirportInfo>& airportInfo)
{
	for (auto it = airportInfo.begin(); it!= airportInfo.end(); it++){
		...
		std::vector<std::pair<int,int>>& current_route = it->second.m_routes;
		std::vector<float>& currentRouteLengths = it->second.m_routeLengths;

		auto calcDistanceUnaer = [&] (std::pair<int, int> m_route) {
			return calculateDistanceBetween(
			it->second.pos[0],
			it->second.pos[1],
			airportInfo[m_route.first].pos[0],
			airportInfo[m_route.first].pos[1] );
		};

/*		for (auto VecIt: current_route){
			currentRouteLengths.push_back(calculateDistanceBetween(
			it->second.pos[0],
			it->second.pos[1],
			airportInfo[VecIt.first].pos[0],
			airportInfo[VecIt.first].pos[1] ));

		}
		*/
		currentRouteLengths.resize(current_route.size());	
		std::transform(current_route.begin(), current_route.end(), currentRouteLengths.begin(), calcDistanceUnaer);
...
void calculateAverageRouteDistances(std::map<int, AirportInfo>& airportInfo)
{
	...
	for (auto it = airportInfo.begin(); it!=airportInfo.end(); it++){
		it->second.m_averageRouteLength = (float) std::accumulate(it->second.m_routeLengths.begin(), it->second.m_routeLengths.end(), 0)/it->second.m_routeLengths.size();
	}
}
...
void printResults(std::map<int, AirportInfo>& airportInfo)
{
	...
	{
		//std::cout << airport.second.m_routeLengths.size() << std::endl;
